...
| 2 | `sigma-cli` failed due to SSL certificate verification error when fetching MITRE ATT&CK data on import | Could not convert Sigma rules without workaround | Fixed via `pip-system-certs` |
...
1. **Implement account creation alerting early** — Alert on *any* Event 4720 regardless of group membership, not only when followed by 4732. Provides early warning before privilege escalation occurs.
2. **Automate containment** — Add a Kibana alert action (webhook or script) to automatically disable newly created accounts pending review, reducing dwell time.
...
4. **Periodic account audits** — Schedule a weekly script that compares local accounts against an approved baseline and alerts on any new entries.
5. **Allow-list known provisioning accounts** — Add `SubjectUserName` exceptions for IT provisioning scripts to reduce false positives on the detection rule.
6. **Validate Sigma rule query logic in Discover first** — Before creating a Kibana rule, test the query in Kibana Discover to catch AND/OR logic errors early and verify field names match the actual index mapping.
7. **Implement LAPS** — Local Administrator Password Solution gives every endpoint a unique randomized local admin password, preventing credential reuse even if a local admin account is compromised.
