...
The goal of this case is not to gain access, but to validate that the lab can (1) capture the noisy authentication failures, (2) surface them in Elastic (Kibana), and (3) detect and respond using an Elastic Security detection rule and optional firewall containment.
...
5. Kibana (Discover + Alerts view) shows both the raw events and the correlated alerts for the brute-force window.
...
- Provide a foundation for additional rules around other logon services (RDP, VPN, web logins).
...
- Extend the same pattern to additional services (RDP, VPN, web logins) using similar telemetry and rule logic.