...
Nmap sent ICMP timestamp requests, TCP window size probes, and unusual flag combinations to fingerprint the OS. Result: **Microsoft Windows 10 1803 at 97% confidence**. The OS CPE (`cpe:/o:microsoft:windows_10`) allows an attacker to narrow down applicable CVEs.

### Step 3 — Service Version Detection (7.77 seconds)

Nmap probed ports 80, 443, 445, and 3389 for service banners. **Sysmon Event ID 3 (real, confirmed at 03:06:50):** The RDP probe was logged by Sysmon on the victim. `svchost.exe` (NT AUTHORITY\NETWORK SERVICE) received an inbound TCP connection from `172.16.0.11` on port 3389.

### Step 4 — Critical Finding: SMB + RDP Both Externally Visible

The scan revealed **both SMB (445) and RDP (3389) are reachable** from the attacker's network segment. This establishes a realistic **kill chain**: Recon (INC-005) → Credential Attack (INC-004 pattern) → potential Remote Access.

### Step 5 — No Follow-On Exploitation

No authentication attempts, exploit payloads, or lateral movement were observed after the scan. Case classified as **pure reconnaissance**.
